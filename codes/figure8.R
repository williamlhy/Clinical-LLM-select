...
#cols <- c("Pretrain"= "#000000FF","Fine-tune" = "#595959FF","Inference"="#B3B3B3FF")
...
plot_gpu <- function(tt){
  ...
  alpha_vector <- c("Pretrain"=1,"Fine-tune"=0.65,"Inference"=0.3)
  ...
  p1 <- ggplot(gpu2, aes(x=as.factor(id), y=ln_memory)) +       # Note that id is a factor. If x is numeric, there is some space between the first bar
    ...
    scale_alpha_manual(values = alpha_vector ,breaks=c("Pretrain","Fine-tune","Inference"),guide="none") +
    ...
    #scale_pattern_alpha_manual(values = alpha_vector, breaks=c("Pretrain","Fine-tune","Inference"), name="Process") +
...
p9 <- ggplot(tdp2, aes(x=as.factor(id), y=ln_TDP)) +       # Note that id is a factor. If x is numeric, there is some space between the first bar
  ...
  scale_fill_manual(values = c("black","black","black","black","black","black","black","black"),name="Tasks of LLMs in clinical settings"
                    ,labels=c("1. Disease diagnosis","2. Disease prediction","3. Information extraction","4. Medical image processing",
                              "5. Textual question answering","6. Multimodal question answering","7. Text generation","8. Text summarization" ) ) +
  #scale_alpha_manual(values = alpha_vector, breaks=c("Pretrain","Fine-tune","Inference"), name="Process") +
...
p10 <- ggplot(tdp2, aes(x=as.factor(id), y=ln_TDP)) +       # Note that id is a factor. If x is numeric, there is some space between the first bar
  ...
  scale_fill_manual(values = c("black","black","black","black","black","black","black","black"),name="Tasks of LLMs in clinical settings"
                    ,labels=c("1. Disease diagnosis","2. Disease prediction","3. Information extraction","4. Medical image processing",
                              "5. Textual question answering","6. Multimodal question answering","7. Text generation","8. Text summarization" ) ,guide="none") +
  scale_alpha_manual(values = alpha_vector, breaks=c("Pretrain","Fine-tune","Inference"), name="Process") +
  theme(legend.key.size = unit(0.6, 'cm'), #change legend key size
...
plots2 <- cowplot::ggdraw() +
  cowplot::draw_plot(plots, x = 0, y = 0, width = 0.73, height = 0.98,scale=0.98) + 
...
ggsave(plot=plots2, file="plots3/fig3_leg.pdf", width = myw, height = myh)
ggsave(plot=plots2, file="plots3/fig3_leg.png", width = myw, height = myh, bg = "white")
